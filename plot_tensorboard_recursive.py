...
def plot_combined_metrics(run_data, output_dir='plots'):
    ...
    # Define color mapping for topologies
    topology_colors = {
        'time-varying': 'green',
        'exponential': 'blue',
        'ring': 'orange',
        ...
        for var in variables:
            ...
            for run_dir, data in group_data.items():
                if var in data['data']:
                    ...
                    color = topology_colors.get(topology, 'black')  # Default to black if topology not in mapping
                    plt.plot(steps, values, label=f"{topology}", color=color)
...
def find_event_files(base_dir):
    """Recursively find all event files in the given directory."""
    event_dirs = []
    
    # Check if the base_dir itself contains event files
    if glob.glob(os.path.join(base_dir, 'event*')):
        event_dirs.append(base_dir)
    ...
    for root, dirs, files in os.walk(base_dir):
        for file in files:
            if file.startswith('event'):
                ...
                break  # Only add each directory once
...
if __name__ == '__main__':
    ...
    run_dirs = []
    if args.run_dirs:
        # Use specified run directories and find event files recursively
        for run_dir in args.run_dirs:
            if os.path.exists(run_dir):
                # Find all subdirectories with event files
                event_dirs = find_event_files(run_dir)
                if event_dirs:
                    run_dirs.extend(event_dirs)
                else:
                    print(f"No event files found in {run_dir} or its subdirectories")
            else:
                print(f"Run directory not found: {run_dir}")
    else:
        # Use all run directories in the 'runs' folder
        for run_dir in glob.glob('runs/*'):
            event_dirs = find_event_files(run_dir)
            if event_dirs:
                run_dirs.extend(event_dirs)
